...
const live = new Map<Table, readonly string[]>([
  ['issue', j.issue],
  ['comment', j.comment],
]);
...
const log = generateLog(
  WORKLOAD,
  Number(process.env.LOG_MB ?? 24) * 1048576,
  0xc0ffee,
  0,
...
class Direct {
  ...
  insert(t: string, cols: readonly string[], vals: unknown[]) {
    const s = this.#shape(t, cols);
    vals.push(this.#version);
    s.insert.run(vals);
  }
  update(t: string, cols: readonly string[], vals: unknown[], key: unknown) {
    const s = this.#shape(t, cols);
    vals.push(this.#version, key);
    if (s.update.run(vals).changes === 0) {
      vals.length -= 2;
      this.insert(t, cols, vals);
    }
  }
  delete(t: string, key: unknown) {
    let s = this.#dels.get(t);
    if (!s) {
      s = this.#db.prepare(`DELETE FROM "${t}" WHERE "id"=?`);
      this.#dels.set(t, s);
    }
    s.run(key);
  }
...
for (let rep = 0; rep < 3; rep++) {
  ...
  for (const e of entries) {
    ...
    if (m[0] === 'begin') {
      a.begin(m[2].commitWatermark);
      continue;
    }
    if (m[0] === 'commit') {
      a.commit(m[2].watermark);
      continue;
    }
    const c = m[1],
      table = c.relation.name;
    if (c.tag === 'delete') {
      a.delete(table, c.key.id);
      applied++;
      continue;
    }
    const row = c.new;
    ...
    if (c.tag === 'insert') a.insert(table, cols, vals);
    else a.update(table, cols, vals, row.id);
    applied++;
  }
  ...
  for (const spec of SPECS) {
    const [r] = db
      .prepare(
        `SELECT COUNT(*) n, SUM(LENGTH(COALESCE("id",''))) i, SUM(LENGTH(COALESCE("created",0))) c, COUNT(DISTINCT "_0_version") v FROM "${spec.name}"`,
      )
      .all<{n: number; i: number; c: number; v: number}>();
  ...
  console.log(
    `applied=${applied}  ${ms.toFixed(1)} ms  ${((ms * 1000) / applied).toFixed(2)} us/change  checksum=${parts.join(' ')}`,
  );
  db.close();
