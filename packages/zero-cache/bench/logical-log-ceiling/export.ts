...
const mkProc = (db: Database) =>
  new ChangeProcessor(new StatementRunner(db), 'backup' as never, (_, e) => {
    throw e;
  });
...
if (!existsSync(BASE)) {
  ...
  for (const s of SPECS) {
    proc.processMessage(lc, ['data', createTableMessage(s)]);
  }
    ...
    for (; seq < stop; seq++) {
      ...
      proc.processMessage(lc, [
        'data',
        {
          ...
          new:
            t === 'issue' ? issueRow(rand, id, seq) : commentRow(rand, id, seq),
        },
      ]);
    }
  ...
  writeFileSync(
    `${BASE}.ids.json`,
    JSON.stringify({issue: live.get('issue'), comment: live.get('comment')}),
  );
  console.error(`base built: ${ROWS} rows`);
} else {
  const j = JSON.parse(readFileSync(`${BASE}.ids.json`, 'utf8'));
  live.set('issue', j.issue);
  live.set('comment', j.comment);
}

const WORKLOAD = (process.env.WORKLOAD ?? 'mixed') as Parameters<
  typeof generateLog
>[0];
...
function rec(op: number, table: number): Buffer {
  ...
  b.writeUInt8(op, 0);
  b.writeUInt8(table, 1);
  recs.push(b);
  return b;
}
function setStr(b: Buffer, slot: number, v: unknown, nullBit: number) {
  if (v === null || v === undefined) {
    b.writeUInt16LE(b.readUInt16LE(2) | (1 << nullBit), 2);
    return;
  }
  const [off, len] = put(String(v));
  b.writeUInt32LE(off, 8 + slot * 8);
  b.writeUInt32LE(len, 12 + slot * 8);
}
function setInt(b: Buffer, base: number, slot: number, v: unknown) {
  b.writeBigInt64LE(
    BigInt(typeof v === 'boolean' ? (v ? 1 : 0) : Math.trunc(Number(v ?? 0))),
    base + slot * 8,
  );
}
...
for (const json of entries) {
  ...
  if (msg[0] === 'begin') {
    const b = rec(OP.BEGIN, 0);
    setStr(b, 0, msg[2].commitWatermark, 0);
    continue;
  }
  if (msg[0] === 'commit') {
    const b = rec(OP.COMMIT, 0);
    setStr(b, 0, msg[2].watermark, 0);
    continue;
  }
  const c = msg[1];
  const table = c.relation.name === 'issue' ? 0 : 1;
  if (c.tag === 'delete') {
    const b = rec(OP.DELETE, table);
    setStr(b, 0, c.key.id, 0);
    nData++;
    continue;
  }
  const row = c.new;
...
console.log(
  `records=${recs.length} data=${nData} json=${(log.bytes / 1048576).toFixed(1)}MB bin=${(out.length / 1048576).toFixed(1)}MB arena=${(arenaLen / 1048576).toFixed(1)}MB`,
);
